<?php namespace Subscribo\Omnipay\Shared\Exception;

use Subscribo\Omnipay\Shared\Exception\NotSuccessfulResponseHttpMessageSendingException;

/**
 * Class ClientErrorResponseHttpMessageSendingException
 *
 * @package Subscribo\OmnipaySubscriboShared
 */
class ClientErrorResponseHttpMessageSendingException extends NotSuccessfulResponseHttpMessageSendingException {}
